...
(function (root) {
  ...
  const SCHEMA_VERSION = 'EV1';
  const FLOW_VERSION = 'F1-2026-09-25';
  const EVENTS = {
    ...
    result_view: ['hero_id', 'alt_ids', 'eligible', 'ms_to_result', 'answers'],
    ...
    feedback_view: ['hero_id'], feedback_answer: ['helped', 'hero_id'], feedback_text: ['helped', 'text_length'],
  ...
  function track(event, props) {
    ...
    if (endpoint && navigator.sendBeacon) {
      navigator.sendBeacon(endpoint, new Blob([JSON.stringify(e)], { type: 'application/json' }));
    } else {
